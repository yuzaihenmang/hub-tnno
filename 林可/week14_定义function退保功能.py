...
def cancel_insurance(policy_id: str, reason: str, insured_name: str):
    """
    办理退保业务
    ...
    Args:
        ...
        insured_name: 投保人姓名
    """
    # 简化的退保逻辑，实际场景会对接保单系统校验保单有效性
    valid_policy_prefixes = ["life", "health", "accident"]
    if not policy_id.startswith(tuple(valid_policy_prefixes)) or len(policy_id) < 8:
        return json.dumps({
            "error": "保单编号无效",
            "note": "有效保单编号需以life/health/accident开头，且长度不少于8位"
        }, ensure_ascii=False)

    # 退保金额计算（简化逻辑）
    product_type = policy_id.split("_")[0]
    if product_type == "life_001":
        # 人寿保险退保返还已交保费的70%（示例）
        refund_ratio = 0.7
        refund_note = "人寿保险退保返还已交保费的70%，扣除工本费50元"
    elif product_type == "health_001":
        # 医疗保险退保返还已交保费的50%（示例）
        refund_ratio = 0.5
        refund_note = "医疗保险退保返还已交保费的50%，无工本费"
    elif product_type == "accident_001":
        # 意外险退保返还已交保费的80%（示例）
        refund_ratio = 0.8
        refund_note = "意外险退保返还已交保费的80%，扣除工本费20元"
    else:
        refund_ratio = 0.0
        refund_note = "未知产品类型，无法计算退保金额"

    # 模拟已交保费（实际需从保单系统获取）
    simulated_paid_premium = 5000  # 模拟已交保费5000元
    base_refund = simulated_paid_premium * refund_ratio
    # 扣除工本费
    if product_type == "life_001":
        final_refund = base_refund - 50
    elif product_type == "accident_001":
        final_refund = base_refund - 20
    else:
        final_refund = base_refund

    result = {
        "policy_id": policy_id,
        "insured_name": insured_name,
        "cancel_reason": reason,
        "simulated_paid_premium": simulated_paid_premium,
        "refund_ratio": f"{refund_ratio * 100}%",
        "final_refund_amount": round(final_refund, 2),
        "notes": [
            refund_note,
            "退保办理时效为3-5个工作日，退款将打入您绑定的银行卡",
            "退保后保障立即终止，且可能影响后续投保该公司产品的费率"
        ]
    }
...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "办理保险退保业务，计算退保金额并返回退保相关说明",
            "parameters": {
                "type": "object",
                "properties": {
                    "policy_id": {
                        "type": "string",
                        "description": "保单编号，有效编号需以life/health/accident开头，长度不少于8位"
                    },
                    "reason": {
                        "type": "string",
                        "description": "退保原因，例如：经济压力大、保障需求变更、产品不合适等"
                    },
                    "insured_name": {
                        "type": "string",
                        "description": "投保人姓名"
                ...
                "required": ["policy_id", "reason", "insured_name"]
            }
        }
    }
]
...
def demo_scenarios():
    ...
    scenarios = [
        ...
        "我要退保，保单号是life_0011234，姓名张三，原因是经济压力大"
    ]
...
# 示例6：退保
run_agent("我要退保，保单号是life_0011234，姓名张三，原因是经济压力大", model="qwen-plus")
