...
class Dataset:
    ...
    def load(self):
        self.data = []
        print(f"正在加载语料文件: {self.corpus_path}")
...
def main():
    ...
    if not os.path.exists(vocab_path):
        print(f"错误: 字表文件 {vocab_path} 不存在!")
        return
    if not os.path.exists(corpus_path):
        print(f"错误: 语料文件 {corpus_path} 不存在!")
        return
...
def predict(model_path, vocab_path, input_strings):
    ...
    if not os.path.exists(model_path):
        print(f"错误: 模型文件 {model_path} 不存在! 请先运行训练。")
        return

    vocab = build_vocab(vocab_path)  # 建立字表
    model = TorchModel(char_dim, hidden_size, num_rnn_layers, vocab)  # 建立模型
    model.load_state_dict(torch.load(model_path, weights_only=False, map_location='cpu'))  # 加载训练好的模型权重
    model.eval()
